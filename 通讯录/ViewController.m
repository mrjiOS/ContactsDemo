...
@property (nonatomic, strong)SouSuoTableView *selectTable;
...
- (void)viewDidLoad {
    ...
    NSArray *nameArray = @[@"bgd", @"an",@"哈哈", @"abc", @"frd", @"王者", @"懵", @"菠菜", @"阿姨", @"啊嘛",@"*&",@"雅思",@"嘎嘣",@"阿女",@"补水", @"绑定"];
    ...
    //  2.判断首字母在索引里面的位置，添加联系人
    for (NSString *buddy in nameArray) {
        
            NSString *firstLetter = [self pinyinFromChineseString:buddy];
            NSInteger section = [indexCollation sectionForObject:[firstLetter substringToIndex:1] collationStringSelector:@selector(uppercaseString)];
            
            NSMutableArray *array = [[_sectionArray objectAtIndex:section] mutableCopy];
            [array addObject:buddy];
            [_sectionArray replaceObjectAtIndex:section withObject:[NSMutableArray arrayWithArray:array]];
        }
    ...
    for (int i = 0; i < [_sectionArray count]; i++) {
        NSArray *array = [[_sectionArray objectAtIndex:i] sortedArrayUsingComparator:^NSComparisonResult(NSString *obj1, NSString *obj2) {
            NSString *firstLetter1 = [self pinyinFromChineseString:obj1];
            NSString *firstLetter2 = [self pinyinFromChineseString:obj2];
            
            if (![_titleArray containsObject:[firstLetter1 substringToIndex:1]]) {
                [_titleArray addObject:[firstLetter1 substringToIndex:1]];
            }

            
            return [firstLetter1 compare:firstLetter2 options:NSNumericSearch] == NSOrderedDescending;
            
            
        }];
        
        
        [_sectionArray replaceObjectAtIndex:i withObject:[NSMutableArray arrayWithArray:array]];
    }
    
    //去掉空的section
    for (NSInteger i = [_sectionArray count] - 1; i >= 0; i--) {
        NSArray *array = [_sectionArray objectAtIndex:i];
        if ([array count] == 0) {
...
{
    NSString *name = [NSString stringWithFormat:@"%@", _titleArray[section]];
    return name;
}
